...
public class MessageContract extends BaseContract {
    ...
    private static int seqNumColumn = -1;

    public static long getSeqNum(Cursor cursor) {
        if (seqNumColumn < 0) {
            seqNumColumn = cursor.getColumnIndexOrThrow(SEQUENCE_NUMBER);
        }
        return cursor.getLong(seqNumColumn);
    }

    public static void putSeqNum(ContentValues out, long seqNum) {
